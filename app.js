...
const addCartToHTML = ()=>{
    listCartHTML.innerHTML=''; let totalQuantity=0, totalPrice=0;
    if(cart.length>0){
        cart.forEach(item=>{
            totalQuantity+=item.quantity;
            let pos = products.findIndex(p=>p.id==item.product_id);
            let info = products[pos];
            totalPrice += item.quantity*info.price;
            let newItem = document.createElement('div');
            newItem.classList.add('item'); newItem.dataset.id=item.product_id;
...
const changeQuantityCart = (product_id,type)=>{
    let pos = cart.findIndex(v=>v.product_id==product_id);
    if(pos>=0){
        if(type==='plus'){ cart[pos].quantity+=1; }
        else{ let newQty = cart[pos].quantity-1; if(newQty>0) cart[pos].quantity=newQty; else cart.splice(pos,1); }
    }
    addCartToHTML(); addCartToMemory();
};

// Checkout & Name modal with dynamic total & Discord webhook
const checkout = ()=>{
    if(cart.length<1){ showNotificationBox('Your cart is empty!'); return; }
    ...
    const totalCart = cart.reduce((sum,item)=>{
        ...
        return sum + (info ? item.price*item.quantity : 0);
    },0);
    const totalAmountSpan = document.getElementById('totalAmount');
    if(totalAmountSpan) totalAmountSpan.textContent = 'RM '+totalCart.toFixed(2);
    ...
    document.getElementById('submitRoom').onclick = ()=>{
        ...
        if(!customerPhone){ showNotificationBox('Phone is required'); return; }
        if(digitsOnly.length<10){ showNotificationBox('Phone must be 10+ digits'); return; }
        ...
        fetch(discordWebhookURL,{
            ...
            body:JSON.stringify({
                ...
                embeds:[{
                    ...
                    color:7506394,
                    fields:[
                        ...simplifiedCart.map(item=>({ name:item.name, value:`Quantity: ${item.quantity} | Price: RM${item.price}`, inline:false })),
        ...
        .then(()=>{
            ...
            cart=[]; addCartToHTML(); addCartToMemory();
        })
        .catch(err=>{
...
viewOrderHistoryBtn.addEventListener('click',()=>{
    ...
    if(!isOpen) orderHistoryContainer.innerHTML='';
    const orderHistory = JSON.parse(localStorage.getItem('orderHistory')) || [];
    if(orderHistory.length===0 && !isOpen) orderHistoryContainer.innerHTML='<p>You have no past orders.</p>';
    else if(!isOpen) orderHistory.forEach((order,i)=>{
        const div = document.createElement('div'); div.classList.add('order-history-item');
        const itemsHTML = order.cart.map(item=>`<li>${item.quantity} × ${item.name} (RM${item.price})</li>`).join('');
        div.innerHTML=`<h3>Order ${i+1} — ${order.date}</h3><p><strong>Room:</strong> ${order.name}</p><p><strong>Phone:</strong> ${order.phone}</p><ul>${itemsHTML}</ul>`;
...
const initApp = ()=>{
    ...
    .then(data=>{
        products = data;
        addDataToHTML();
        if(localStorage.getItem('cart')){ cart=JSON.parse(localStorage.getItem('cart')); addCartToHTML(); }
    })
    .catch(err=>console.error('Error fetching product data:',err));
};
initApp();

// Block back button for Android
