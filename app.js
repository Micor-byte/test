...
iconCart.addEventListener('click', () => {
    if(body.classList.contains('showhistory')) { orderHistoryPanel.classList.remove('open'); body.classList.remove('showhistory'); }
...
const changeQuantityCart=(product_id,type)=>{
    ...
    if(pos>=0){
        ...
        else { cart[pos].quantity--; if(cart[pos].quantity<=0) cart.splice(pos,1); }
    }
    addCartToHTML();
    addCartToMemory();
};
...
const checkout = () => {
    ...
    submitBtn.onclick = () => {
        ...
        .then(()=>{
            ...
            // fully reset file input
            const oldInput=document.getElementById('transferScreenshot');
...
function closeOrderHistory(){orderHistoryPanel.classList.remove('open');body.classList.remove('showhistory');}
